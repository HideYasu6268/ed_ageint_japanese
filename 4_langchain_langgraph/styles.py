"""Sidekickアプリの見た目、コースのブランドカラーを使用: ライトグレーの上にネイビーの
文字、作業中を示すブルー、人間の対応が必要な瞬間を示すゴールド、控えめなアクセントの
パープル。Gradio 6では、theme、css、jsはgr.Blocks()ではなくlaunch()に渡される。"""
...
# このブランドはライトモードなので、Gradioがシステムのダークモード設定に
# 引き寄せられないように誘導する。
# launch(head=JS)経由で配信されるため、アプリがマウントされる前に実行され、
# ダークモードのちらつきが発生しない。
JS = """
<script>
