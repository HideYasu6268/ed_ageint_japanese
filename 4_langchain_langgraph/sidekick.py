"""Sidekick: 自作の評価者(evaluator)ループでラップしたcreate_agentワーカー。

ワーカーは単一のcreate_agent(レイヤー3)である。その周りに、ワーカーの回答を
ユーザーの成功基準と照らして確認する独自のループを実行し、回答を受け入れるか、
フィードバックを添えて再試行させるか、質問としてユーザーに戻すかを判断する。
ミドルウェアは、UIと共有するプランをワーカーに持たせ、PIIや暴走するコストへの
ガードレールを設け、機微なアクションの前には人間の承認を得るための一時停止を
提供する。
"""
...
class TolerateToolErrors(AgentMiddleware):
    """ツールの失敗を実行クラッシュにせず、メッセージとしてモデルに返すことで
    リカバリーできるようにする。ブラウザのような外部世界に触れるツールは
    時々失敗するものである。"""
...
class Sidekick:
    ...
    async def setup(self):
        ...
        self.evaluator = ChatGoogleGenerativeAI(model="gemini-flash-latest").with_structured_output(EvaluatorOutput)
...
needs clarification, or seems stuck. Give brief, concrete feedback."""
    ...
    async def run_turn(self, message: str, success_criteria: str, history: list) -> list:
        """会話の1ターン分: ワーカーがタスクに取り組み、評価者がそれを確認する。
        MAX_ATTEMPTSまでフィードバックを添えて再試行する。ワーカーが承認待ちで
        一時停止した場合は、pausedをセットした状態で即座に戻り、resume()が
        同じターンを続行する。"""
        self.task = message
    ...
    async def resume(self, history: list) -> list:
        """ワーカーが一時停止していたアクションを承認し、ターンを続行する。"""
    ...
    def cleanup(self):
        """MCPサーバーを停止する。これによりブラウザウィンドウが閉じる。"""
        if self.sessions:
            self.sessions.stop()
