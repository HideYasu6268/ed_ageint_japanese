"""Sidekick用のGradioアプリ。実行方法: uv run app.py"""
...
async def process_message(sidekick, message, success_criteria, history):
    if sidekick is None:  # MCPサーバーの起動が完了する前にユーザーがクリックした場合
...
def watch_todos(sidekick):
    """タイマーから呼び出される。タイマーはプランパネルの唯一の書き込み元である。
    もし時間のかかるイベントがこのパネルを出力として持っていたら、Gradioはそれを
    保留状態としてロックしてしまい、Sidekickが作業している間にライブ更新が
    表示されなくなってしまう。"""
