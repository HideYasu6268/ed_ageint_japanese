"""Agnoのワーカーを、単純なサブプロセスとしてボードに対して実行する。

単体で実行すると、自分自身のDay3のゴール(notes.txtを読み、スペイン語に翻訳し、spanish.txtに
書き出す)を仕込んで作業する。ボード上にステップを計画し、filesystem MCPサーバー経由でファイルを
読み、翻訳し、スペイン語の結果を書き戻し、ゴールを閉じる前に各ステップをチェックしていく。

タスクidと共有ボードのパスを渡すと、同じワーカーがDay5のエージェントループに参加するように
なる。ボードとファイルツールを共有ボードと共有の場所に向け、その1つのタスクを取り込み、
タスクが求めるものを組み立て、終わったら終了する。ボードの他の部分には手を出さない。

    uv run agno_worker.py                       # 単体でのDay3デモ
    uv run agno_worker.py <taskId> <boardPath>  # Day5: 共有ボード上の1タスクを作業する
"""
...
# Day5モードは "<taskId> <boardPath>" の形。boardをimportする前にこれを読む必要がある。
# boardはimport時にBOARD_PATHからファイルを選ぶためである。単体実行時はこの処理は動かない。
...
# AgnoのMCPツールはサーバーのstderrを公開しないため、そのstdioクライアントをDEVNULLに
# 向けている。これによりfilesystemサーバーの起動時バナーが静かになり、Windows上のJupyterカーネル
# からも実行できるようになる。今週の各フレームワークで必要になる、同じ対処である。
...
def seed() -> int:
    """ボードをリセットし、古い出力があれば消し、1つのゴールを追加する。"""
    board.reset_board()
...
async def main() -> None:
    if TASK_ID is None:
        ...
        board.claim_todo(TASK_ID)  # 共有ボード上のこの1タスクだけを取り込む
        message = (
    ...
    if TASK_ID is None:  # 単体実行時は結果を表示する。Day5ではオーケストレーターがコンソールを管理する
        print("\nBoard after the run:")
        board.show_board()
