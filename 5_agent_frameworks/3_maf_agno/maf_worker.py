"""Microsoft Agent Frameworkのワーカーを、サブプロセスとしてボードに対して実行する。
...
タスクが求めるものを組み立て、終わったら終了する。ボードの他の部分には手を出さない。
    ...
    uv run maf_worker.py                       # 単体でのDay3デモ
    uv run maf_worker.py <taskId> <boardPath>  # Day5: 共有ボード上の1タスクを作業する
"""
...
warnings.filterwarnings("ignore", message=r".*experimental.*")  # MAFのimport時の通知を静かにする
...
async def main() -> None:
    ...
    filesystem = MCPStdioTool(
        ...
        cwd=str(WORK_DIR),  # work dir内でサーバーを起動し、相対ファイル名がそこで解決されるようにする
    )
