"""Pydantic AIのワーカーを、単純なサブプロセスとしてボードに対して実行する。

単体で実行すると、自分自身のDay2のゴール(notes.txtを読み、スペイン語に翻訳し、spanish.txtに
...
タスクが求めるものを組み立て、終わったら終了する。ボードの他の部分には手を出さない。
    ...
    uv run pydantic_worker.py                       # 単体でのDay2デモ
    uv run pydantic_worker.py <taskId> <boardPath>  # Day5: 共有ボード上の1タスクを作業する
"""
