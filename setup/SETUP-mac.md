...
Macでの開発が初めての場合は、XCode developer toolsのインストールが必要になることがあります。[こちらの手順](https://chatgpt.com/share/67b0b8d7-8eec-8012-9a37-6973b9db11f5)を参照してください。
...
重要な注意: Cursorは最近、起動後のデフォルト画面として新しい「Agents」スプラッシュ画面を表示するようになりました。Command+Shift+Nで通常の画面に戻れます。この仕様が変わってほしいものです!詳細はこちらのQ54で:  
https://edwarddonner.com/avatar?q=54

### パート2: リポジトリのクローン

1. Cursorで新しいウィンドウを開く(File >> New WindowまたはCommand+Shift+N)

2. そのウィンドウでターミナルを開く(View >> TerminalまたはCtrl+バッククォート)

3. `git --version` でgitがインストールされていることを確認してください。インストールされていない場合は、上部の説明にあるようにXCode developer toolsをインストールしてください
...
拡張機能ウィンドウを開いてください(Command+Shift+XまたはViewメニュー >> Extensions)。  
...
`pwd` と入力して現在のディレクトリを確認し、'agents'ディレクトリにいることを確認してください。私の場合は`/Users/ed/projects/agents`ですが、あなたの場合も似たようなパスになるはずです。
...
3. 「Path to folder with a list of Virtual Environments」というフィールドに、プロジェクトのルートのパス(例: /Users/username/projects/agents)を入力してください  
そして再度試してください。
